  ...
  success: function(arr){
    ...
    for(var i = 0; i < arr.length; i++){
      str += ` <li class="goods_item">
        ...
        <div class="sa">
            抢购价:<span>${arr[i].price}</span>
        </div>
        <div class="sc">
            <div><span id="${arr[i].id}" class="sc_btn">加入购物车</span><span class="sc_Btn"><a href="./indent.html">立即购买</a></span></div>
        </div>
    </li>`
    }
...
$(".goods_box ul").on("click", '.sc_btn', function(){
  ...
  if(first){
    ...
    if(!same){
      var obj = {id: id, num: 1, price:aprice};
      cookieArr.push(obj);
    }
